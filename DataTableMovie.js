...
export default class MatDataTable extends Component {
  ...
  componentDidMount(prevProps) {    
    ...
    .then(res => {
      console.log(res);
      // "data' is the hole thing of return  and "data" is all record information
   
      this.setState({ person: res.data.data});
      
      // mapping array
      // used funtion passing val parameter
      var newArr = res.data.map(val => { 
  ...
  render() {
    return (      
      ...
      /* ALL COLUMN OUTPUT  */ 
          ...
          //  { title: "category", field: "category", type: "category" }  ,            
            { title: "portrayed", field: "portrayed", type: "portrayed" } ,   
            { title: "status", field: "status", type: "status" } ,               
           { title: "char_id", field: "char_id", type: "char_id" } ,  
          ]}
